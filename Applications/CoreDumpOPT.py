...
def createRecipe(podID: str, binFaces: list, binLablels: list, binList: list, header: str = _C_DATA_HEADER_, mode: str = 'FBA') -> None:
    recipe: str = f"{header}"
    print(binList)
    recipe += ''.join([f">{binFaces[count]}{binLablels[count]}{binList[count].replace('-', '*')}" for count in range(len(binLablels))]) if mode == 'FBA' else ''.join([f">{binFaces[count]}{binLablels[count]}{binList[count].group(0).replace('-', '*')}" for count in range(len(binLablels))])
    recipe += "!"
